...
<?php
	...
	foreach ($guides['adopted_guides'] as $key => $array) {
		$published_guides[] = ['id' => $array['id'], 'title' => $array['title']];
	}

	foreach ($guides['orphaned_guides'] as $key => $array) {
		$published_guides[] = ['id' => $array['id'], 'title' => $array['title']];
	}
	function cmp($a, $b)
	{
...
<div class="page-container row-fluid">
    ...
    <div class="page-content">
        ...
        <div class="content">
			<form name="guideform" id="guideform" method="post" enctype="multipart/form-data" action="<?=SITE_URL; ?>guideline/linkguideedit">
								...
								<div class="row">
									<div class="">
										<div class="form-group">
											...
											<div class="controls">
												<input class="form-control" type="text" id="guidename" name="guidename" value="<?=$data[0]['title']; ?>" disabled>
											</div>
										</div>
										<div class="form-group">
											<label class="form-label">Description</label>
											<div class="controls">
												<textarea class="text-editor form-control" id="guidedescription" name="guidedescription" disabled><?=$data[0]['text']; ?></textarea>
												<select id='selectguidelines' name='selectguidelines' disabled>
										...
										<div class="form-group">
											...
											style="display: none;" type="text" id="guidecategories" name="guidecategories" value="<?=$data[0]['categories']; ?>" disabled>
										</div>
										<div class="form-group">
											...
											<div class="controls">
												...
												foreach ($data[0]['files'] as $key => $item) {
													echo "<input type='checkbox' name='filedelete[]' value='{$item['filename']}' disabled>&nbsp;" . ($item['dl_filename'] !== '' ? $item['dl_filename'] : $item['filename']) . '<br />';
												}
												?>
												<input type="file" class="" style="line-height:15px; border:0" id="guidefile" name="guidefile" disabled>
											</div>
										</div>
										<hr />
										...
										<div class="form-group">
											...
											<div class="controls">
												<input type="text" class="form-control" id="guidewarningtitle" name="guidewarningtitle" value="<?=$data[0]['warningtitle'] ?? 'Warning'; ?>" onkeyup="$('#headerspan0').html($(this).val().toLowerCase().replace(/\b[a-z]/g, function(letter) {return letter.toUpperCase(););" disabled>
											</div>
										</div>
										<div class="form-group">
												<label class="form-label">Warning content</label>
												<div class="controls">
													<textarea class="text-editor form-control" id="guidewarning" name="guidewarning" disabled><?=$data[0]['warning']; ?></textarea>
												</div>
										</div>
										</div>
										<h4>Citation References</h4>
										<?php if (!isset($data[0]['citations'])) { ?>

											<div class="citation-wrapper">
												<div class="input-group">
													<span class="input-group-addon"></span>
													<input type="text" class="form-control" placeholder="Author" name="citationauthor[]" value="" disabled>
												</div>
												<br />
												<div class="input-group">
													<span class="input-group-addon"></span>
													<input type="text" class="form-control" placeholder="Citation Reference" name="citationreference[]" value="" disabled>
												</div>
												<br />
											</div>
										...
										<?php } else { ?>
											<?php foreach ($data[0]['citations'] as $key => $value) {
													...
													<div class="citation-wrapper">
														<input type="hidden" name="citationparentguidelineid[]" value="<?=$value['parent_guideline_id']; ?>" disabled>
														<div class="input-group">
															<span class="input-group-addon"></span>
															<input type="text" class="form-control" placeholder="Network" name="citationnetwork[]" value="<?=$value['network']; ?>"  disabled>
														</div>
														<br>
														<div class="input-group">
															<span class="input-group-addon"></span>
															<input type="text" class="form-control" placeholder="Author" name="citationauthor[]" value="<?=$value['author']; ?>"  disabled>
														</div>
														<br />
														<div class="input-group">
															<span class="input-group-addon"></span>
															<input type="text" class="form-control" placeholder="Citation Reference" name="citationreference[]" value="<?=$value['reference']; ?>"  disabled>
														</div>
														<br />
													</div>
												<?php
										...
										<?php } ?>
										<button class="btn btn-primary" id="repeat-citation-btn" type="button" disabled>Add Citation</button>
										<br />
										<hr />
										<input class="btn btn-primary savebutton" type="submit" value="Save Changes" disabled>
									</div>
								</div>
							</div>
						</div>
									...
									<div id="content-holder">
										...
										<?php if (!isset($data[0]['contents'])) { ?>
												...
												<div class="custom-field-container alert alert-success" style="">
													<a class="expandlink" data-id="0" href="#">+</a>&nbsp;<span id="headerspan0"></span><button class="close" onclick="if (confirm('You are about to delete this content, please click okay to confirm') == true) {var editor = $('#editor0').ckeditorGet();CKEDITOR.remove(editor);$(this).closest('.content-wrapper').remove();$(this).parent().alert('close');}return false;" disabled></button>
													<div id='contents0'>
														<div class="form-group">
															<label class="form-label">Title</label>
															<div class="controls">
																<input type="text" class="form-control"  name="customcontenttitle[]" value="" onkeyup="$('#headerspan0').html($(this).val().toLowerCase().replace(/\b[a-z]/g, function(letter) {return letter.toUpperCase(););" disabled>
															</div>
														</div>
														<div class="form-group">
															<label class="form-label">Content</label>
															<div class="controls">
																<textarea class="text-editor form-control" id="editor0" name="customcontent[]" disabled></textarea>
																<select class='dinamicguidelineselector' data-id="0" disabled>
																<option value='0'>Add link to another guideline</option>
																<?php
																	foreach ($published_guides as $key => $row) {
																		echo "<option value='" . $row['id'] . "'>" . $row['title'] . '</option>';
										...
										<?php } else { ?>
												...
												<div class="custom-field-container alert alert-success" style="">
													...
													<div id='contents<?=$key; ?>' style='display:none;'>
														<div class="form-group">
															<label class="form-label">Title</label>
															<div class="controls">
																<input type="text" class="form-control" name="customcontenttitle[]" value="<?=$value['title']; ?>"  onkeyup="$('#headerspan<?=$key; ?>').html($(this).val().toLowerCase().replace(/\b[a-z]/g, function(letter) {return letter.toUpperCase(););" disabled>
															</div>
														</div>
														<div class="form-group">
															<label class="form-label">Content</label>
															<div class="controls">
																<textarea class="text-editor form-control" id="editor<?=$key; ?>" name="customcontent[]" disabled><?=$value['content']; ?></textarea>
																<select class='dinamicguidelineselector' data-id="<?=$key; ?>" disabled>
																<option value='0'>Add link to another guideline</option>
																<?php
																	foreach ($published_guides as $key => $row) {
																		echo "<option value='" . $row['id'] . "'>" . $row['title'] . '</option>';
											...
											<?php } ?>
										<?php } ?>
										<button class="btn btn-primary" id="repeat-content-btn" disabled>Add Content</button>
										<br /><br />
										<hr />
										<input class="btn btn-primary savebutton" type="submit" value="Save Changes" disabled>
									</div>
...
<script src="assets/js/pages/guide_link.js" type="text/javascript"></script>
